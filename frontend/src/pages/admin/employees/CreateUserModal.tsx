...
export default function CreateUserModal({ isOpen, onClose, onSuccess }: CreateUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('User created successfully! Login credentials generated.');
      onSuccess();
      onClose();
    } catch (error: any) {
      console.error(error);
      const msg = error.response?.data?.message || error.response?.data || 'Failed to create user.';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6 overflow-y-auto">
              <form id="create-user-form" onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                ...
                {formData.role === 'EMPLOYEE' && (
                  <div className="col-span-2">
                    <label className={`block text-xs font-bold mb-1.5 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>Reporting Manager *</label>
                    <select
                      required name="managerId" value={formData.managerId} onChange={handleChange}
                      className={`w-full rounded-xl border px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${isDark ? 'bg-zinc-900/50 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
                    >
