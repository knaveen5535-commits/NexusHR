              ...
              <tbody className={`divide-y ${isDark ? 'divide-zinc-800' : 'divide-slate-200'}`}>
                {isLoading ? (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${isDark ? 'bg-purple-500/10 text-purple-400 ring-1 ring-inset ring-purple-500/20' : 'bg-purple-100 text-purple-700 ring-1 ring-inset ring-purple-200'}`}>
                        {emp.departmentName || 'N/A'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 text-xs font-semibold ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                        <UserCheck className="h-3 w-3" />
                        {emp.managerName || 'No Manager'}
                      </span>
                    </td>
                    <td className={`px-4 py-3 text-xs font-medium ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{emp.designation}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-bold uppercase ${
                        emp.role === 'ADMIN' ? (isDark ? 'bg-red-500/10 text-red-400 ring-1 ring-inset ring-red-500/20' : 'bg-red-100 text-red-700 ring-1 ring-inset ring-red-200') :
                        emp.role === 'HR' ? (isDark ? 'bg-purple-500/10 text-purple-400 ring-1 ring-inset ring-purple-500/20' : 'bg-purple-100 text-purple-700 ring-1 ring-inset ring-purple-200') :
                        emp.role === 'MANAGER' ? (isDark ? 'bg-blue-500/10 text-blue-400 ring-1 ring-inset ring-blue-500/20' : 'bg-blue-100 text-blue-700 ring-1 ring-inset ring-blue-200') :
                        emp.role === 'NONE' ? (isDark ? 'bg-zinc-500/10 text-zinc-500 ring-1 ring-inset ring-zinc-500/20' : 'bg-slate-100 text-slate-500 ring-1 ring-inset ring-slate-200') :
                        (isDark ? 'bg-slate-500/10 text-slate-400 ring-1 ring-inset ring-slate-500/20' : 'bg-slate-100 text-slate-500 ring-1 ring-inset ring-slate-200')
                      }`}>
                        {emp.role}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium capitalize ${
                        ...
                        (isDark ? 'bg-amber-500/10 text-amber-400 ring-1 ring-inset ring-amber-500/20' : 'bg-amber-100 text-amber-700 ring-1 ring-inset ring-amber-200')
                      }`}>
                        {emp.status}
                      </span>
                    </td>
