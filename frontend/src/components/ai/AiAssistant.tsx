...
export default function AiAssistant() {
  ...
  return (
    <div className="p-4 sm:p-8 h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col rounded-xl border border-border bg-card/50 backdrop-blur-xl overflow-hidden">
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {loading && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
              ...
              <div className="bg-muted border border-border/50">
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              {actions.map((action) => (
                <button
                  ...
                  className="p-3 rounded-lg bg-muted border border-border hover:border-border hover:bg-accent transition-all text-left group"
                >
                  ...
                  <p className="text-[10px] text-muted-foreground line-clamp-1">{action.prompt}</p>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="border-t border-border p-4">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-border bg-background/50 px-4 py-2.5 text-sm text-foreground placeholder-muted-foreground focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors"
            />
            <button
              onClick={() => handleSend()}
              disabled={!input.trim() || loading}
              className="p-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-foreground disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
